...
var buildForm = function buildForm(_ref2) {
  ...
  var Form = function Form(_ref4) {
    ...
    return /*#__PURE__*/jsxRuntimeExports.jsx(reactHookForm.FormProvider, _objectSpread2(_objectSpread2({}, methods), {}, {
      metadata: metadata,
...
function JsonNode({ node, depth, deleteHandle: _deleteHandle, indexOrName, parent, editHandle }) {
    ...
    if (Array.isArray(node) || isObject(node)) {
        ...
        if (type === 'bigint')
            displayValue += 'n';
        const EditingElement = React.useMemo(() => (jsxRuntimeExports.jsx("span", { contentEditable: true, className: className, dangerouslySetInnerHTML: { __html: type === 'string' ? `"${displayValue}"` : displayValue }, ref: valueRef, onKeyDown: handleKeyDown })), [displayValue, type, handleKeyDown]);
        if (type === 'string')
