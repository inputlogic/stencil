...
import { SelectAddDelete } from '../select-add-delete'
...
export const Config = ({}) => {
  ...
  const servers = activeOpenapi?.servers || stencil?.openapi?.servers.reduce((acc, s) => ({...acc, [s.url]: {accounts: []}}), {}) || []
      ...
      <div>
        {stencil &&
          <SelectAddDelete
            ...
            activeOption={{ label: activeServerUrl, canDelete: !stencil.openapi.servers?.some(server => server.url === activeServerUrl) }}
...
const AddAccountForm = ({stencil, config, setConfig, setAccounts, accounts, close}) => {
  console.log('stencil in add acc', stencil)
	const {register, handleSubmit} = useForm()
	const [error, setError] = useState()
  const loginMutation = stencil.useMutation({
    path: '/public/user/login',
    method: 'post',
  }) //mutations.usePublicUserLoginMutation()
	return (
		<form
			className={styles['add-form']}
